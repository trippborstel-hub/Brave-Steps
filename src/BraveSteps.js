...
const BREATHING_TECHNIQUES = [
  {
    name: "Box Breathing",
    icon: "📦",
    description: "Breathe in a square pattern — even and steady",
    phases: [
      { label: "Breathe in", duration: 4, color: "#6C63FF" },
      { label: "Hold", duration: 4, color: "#8B7FFF" },
      { label: "Breathe out", duration: 4, color: "#4ECDC4" },
      { label: "Hold", duration: 4, color: "#44B8B0" },
    ],
  },
  {
    name: "4-7-8 Calm",
    icon: "🌙",
    description: "A longer exhale tells your nervous system to relax",
    phases: [
      { label: "Breathe in", duration: 4, color: "#6C63FF" },
      { label: "Hold", duration: 7, color: "#8B7FFF" },
      { label: "Breathe out", duration: 8, color: "#4ECDC4" },
    ],
  },
  {
    name: "5-5 Wave",
    icon: "🌊",
    description: "Simple and rhythmic, like ocean waves",
    phases: [
      { label: "Breathe in", duration: 5, color: "#6C63FF" },
      { label: "Breathe out", duration: 5, color: "#4ECDC4" },
    ],
  },
];
...
function BreathingExercise() {
  ...
  const technique = BREATHING_TECHNIQUES[techniqueIndex];
  const phase = technique.phases[phaseIndex];
  const totalPhaseDuration = phase ? phase.duration : 1;
  const phaseProgress = phase ? 1 - countdown / totalPhaseDuration : 0;

  const getCircleScale = () => {
    if (!isActive || !phase) return 0.6;
    const isInhale = phase.label === "Breathe in";
    const isExhale = phase.label === "Breathe out";
    const isHold = phase.label === "Hold";

    if (isInhale) return 0.6 + 0.4 * phaseProgress;
    if (isExhale) return 1.0 - 0.4 * phaseProgress;
    if (isHold) {
      const prevPhaseIndex = (phaseIndex - 1 + technique.phases.length) % technique.phases.length;
      const prevPhase = technique.phases[prevPhaseIndex];
      return prevPhase.label === "Breathe in" ? 1.0 : 0.6;
    }
    return 0.8;
  };
  ...
  useEffect(() => {
    ...
    setCountdown(technique.phases[phaseIndex].duration);
  ...
  }, [isActive, phaseIndex, techniqueIndex]);
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
        <h3 style={{ fontFamily: "'Baloo 2', cursive", fontSize: 16, color: "#F0A84E", margin: 0 }}>
          🫁 Breathing Tool
        </h3>
        {isActive && (
          <span
      ...
      {!isActive ? (
        <>
          ...
          >
            Slow breathing activates your parasympathetic nervous system — it's like hitting the brakes on anxiety. Pick
            a pattern:
          </p>
          <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 16 }}>
            {BREATHING_TECHNIQUES.map((tech, i) => (
              <div
                key={i}
                onClick={() => setTechniqueIndex(i)}
                style={{
                  display: "flex",
                  alignItems: "center",
                  gap: 12,
                  padding: "12px 14px",
                  borderRadius: 12,
                  border: techniqueIndex === i ? "2px solid #F0A84E" : "2px solid #f0eff8",
                  background: techniqueIndex === i ? "#fffaf2" : "#fafafe",
                  cursor: "pointer",
                  transition: "all 0.2s",
                }}
              >
                ...
                <div style={{ fontFamily: "'Nunito', sans-serif", fontSize: 11, color: "#bbb", whiteSpace: "nowrap" }}>
                  {tech.phases.map((p) => p.duration).join("-")}
                </div>
              </div>
            ))}
          </div>
          <button
            onClick={start}
            style={{
              background: "linear-gradient(135deg, #F0A84E, #F0C45C)",
              color: "#fff",
              border: "none",
              borderRadius: 12,
              padding: "10px 24px",
              ...
              cursor: "pointer",
              width: "100%",
              boxShadow: "0 3px 12px rgba(240,168,78,0.3)",
            }}
          >
            Start Breathing
          </button>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
          ...
          >
            ...
            <div
              style={{
                ...
                height: 140,
                borderRadius: "50%",
                background: `radial-gradient(circle at 35% 35%, ${phase.color}30, ${phase.color}60)`,
                border: `3px solid ${phase.color}80`,
                display: "flex",
                flexDirection: "column",
                alignItems: "center",
                justifyContent: "center",
                transform: `scale(${scale})`,
                transition: "transform 1s ease-in-out, background 0.5s, border-color 0.5s",
                boxShadow: `0 0 40px ${phase.color}25`,
              }}
            >
              <div
                style={{
          ...
          <div style={{ display: "flex", gap: 6, marginBottom: 16 }}>
            {technique.phases.map((p, i) => (
              <div
                key={i}
                style={{
                  width: i === phaseIndex ? 20 : 8,
                  height: 8,
                  borderRadius: 4,
                  background: i === phaseIndex ? p.color : "#e0dff0",
                  transition: "all 0.3s",
                }}
              />
            ))}
          ...
          >
            {phase.label === "Breathe in"
              ? "Breathe in slowly through your nose. Fill your belly like a balloon."
              : phase.label === "Breathe out"
              ? "Let the air flow out slowly through your mouth. Let your shoulders drop."
              : "Just pause here. You're doing great. Nothing to do but wait."}
          </p>
